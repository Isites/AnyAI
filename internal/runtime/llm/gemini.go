...
func NewGeminiProvider(ctx context.Context, apiKey, baseURL string, headers map[string]string) (*GeminiProvider, error) {
	...
	if httpHeaders := toHTTPHeader(headers); len(httpHeaders) > 0 {
		clientCfg.HTTPOptions.Headers = httpHeaders
	}
