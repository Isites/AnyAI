...
var toolHints = map[string]string{
	"read_file":        "You can read files. It reads file contents, not directory listings. For large or unfamiliar text files, avoid loading the whole file: use query with context_lines to find relevant matches, line_start/line_count to inspect focused ranges, or offset/limit/max_bytes to page through content; continue from next_offset or next_line_start when the result is truncated. Use bash to inspect directories when bash is available. If an image is already attached, inspect that image directly with vision instead of calling read_file or scripts; use read_file for text/file contents and path verification.",
