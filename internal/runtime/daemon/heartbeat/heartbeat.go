...
func (d *Daemon) Start(ctx context.Context) {
	...
	go d.run(ctx)
	runtimelogging.Info("heartbeat daemon started", "interval", d.interval)
}
...
func (d *Daemon) Stop() {
	...
	<-d.done
	runtimelogging.Info("heartbeat daemon stopped")
}
...
func (d *Daemon) tick(ctx context.Context) {
	...
	if err != nil {
		if !os.IsNotExist(err) {
			runtimelogging.Warn("failed to read HEARTBEAT.md", "error", err)
		}
		return
	}
	...
	runtimelogging.Info("heartbeat tick — sending to agent")

	response, err := d.agentFn(ctx, prompt)
	if err != nil {
		runtimelogging.Error("heartbeat agent error", "error", err)
		return
	}

	// Drop HEARTBEAT_OK responses silently
	if strings.TrimSpace(response) == "HEARTBEAT_OK" {
		runtimelogging.Debug("heartbeat — no action needed")
		return
	}

	// Non-OK response means the agent took action — log it
	runtimelogging.Info("heartbeat — agent acted", "response_length", len(response))
}
