...
func (s *Scheduler) Start(ctx context.Context) {
	...
	runtimelogging.Info("cron scheduler started", "jobs", len(s.running))
}
...
func (s *Scheduler) Stop() {
	...
	s.wg.Wait()
	runtimelogging.Info("cron scheduler stopped")
}
...
func (s *Scheduler) Remove(name string) error {
	...
	runtimelogging.Info("cron job removed", "name", name)
	return nil
}
...
func (s *Scheduler) Pause(name string) error {
	...
	runtimelogging.Info("cron job paused", "name", name)
	return nil
}
...
func (s *Scheduler) Resume(name string) error {
	...
	runtimelogging.Info("cron job resumed", "name", name)
	return nil
}
...
func (s *Scheduler) UpdateSchedule(name, newSchedule string) error {
	...
	runtimelogging.Info("cron job schedule updated", "name", name, "schedule", newSchedule)
	return nil
}
...
func (s *Scheduler) runJob(ctx context.Context, job Job) {
	...
	runtimelogging.Info("cron job registered", "name", job.Name, "interval", job.interval)
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			runtimelogging.Info("cron job running", "name", job.Name)
			...
			if err != nil {
				...
				runtimelogging.Error("cron job failed", "name", job.Name, "error", err)
				continue
			}

			runtimelogging.Info("cron job completed", "name", job.Name, "response_length", len(response))
			if job.OutputFn != nil {
