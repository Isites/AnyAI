...
func (w *Watcher) run() {
	...
	for {
		...
		case err, ok := <-w.watcher.Errors:
			...
			runtimelogging.Error("config watcher error", "error", err)
		case <-w.stop:
			return
		}
	}
...
func (w *Watcher) reload() {
	cfg, err := Load(w.path)
	if err != nil {
		runtimelogging.Error("failed to reload config", "error", err)
		return
	}
	runtimelogging.Info("config reloaded", "path", w.path)
	w.callback(cfg)
}
