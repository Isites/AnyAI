...
func NewTelegramChannel(token string, requireMention bool) *TelegramChannel {
	return &TelegramChannel{
		...
		inbound:        make(chan gateway.InboundMessage, 100),
...
// running instance (e.g. anyai start).
...
func (t *TelegramChannel) Connect(ctx context.Context) error {
	t.mu.Lock()
	t.status = gateway.StatusConnecting
	t.mu.Unlock()
...
func (t *TelegramChannel) Disconnect() error {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.status = gateway.StatusDisconnected
	if t.cancel != nil {
		t.cancel()
	}
	return nil
...
func (t *TelegramChannel) Send(ctx context.Context, msg gateway.OutboundMessage) error {
	t.mu.Lock()
	b := t.bot
	t.mu.Unlock()
...
func (t *TelegramChannel) Receive() <-chan gateway.InboundMessage {
	return t.inbound
}

func (t *TelegramChannel) Status() gateway.ChannelStatus {
	t.mu.Lock()
	defer t.mu.Unlock()
	return t.status
}
...
func (t *TelegramChannel) defaultHandler(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	if chatType == gateway.ChatTypeGroup && t.requireMention {
	...
	if chatType == gateway.ChatTypeGroup && t.botUsername != "" {
	...
	if len(msg.Photo) > 0 {
		...
		media = append(media, gateway.MediaAttachment{
			Type:    "photo",
	...
	t.inbound <- gateway.InboundMessage{
		Channel:    "telegram",
