...
func (w *WhatsAppChannel) Connect(ctx context.Context) error {
	w.mu.Lock()
	w.status = gateway.StatusConnecting
	w.mu.Unlock()
...
func (w *WhatsAppChannel) Send(ctx context.Context, msg gateway.OutboundMessage) error {
	w.mu.Lock()
	client := w.client
	w.mu.Unlock()
...
func (w *WhatsAppChannel) Receive() <-chan gateway.InboundMessage {
	return w.inbound
}

func (w *WhatsAppChannel) Status() gateway.ChannelStatus {
	w.mu.Lock()
	defer w.mu.Unlock()
	return w.status
}
...
func (w *WhatsAppChannel) handleMessage(evt *events.Message) {
	...
	w.inbound <- gateway.InboundMessage{
		Channel:    "whatsapp",
		AccountID:  accountID,
		ChatType:   chatType,
		SenderID:   senderID,
